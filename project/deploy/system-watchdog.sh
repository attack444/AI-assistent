#!/usr/bin/env bash
# Watchdog: 5mb2 + AI Helper + панель/API/DeepSeek.
...
export WATCHDOG_PANEL_URL="${WATCHDOG_PANEL_URL:-http://127.0.0.1:3000}"
export WATCHDOG_ALLOW_RESTART="${WATCHDOG_ALLOW_RESTART:-1}"
export WATCHDOG_ASK_DEEPSEEK="${WATCHDOG_ASK_DEEPSEEK:-1}"
# Ремонт инфраструктуры — DeepSeek, не tiny free model
...
# Предпочитаем API (localhost без Bearer)
