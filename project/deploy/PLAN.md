...
| DNS A → VPS | ок |
| http://5mb2.ru | **ок** |
| https://5mb2.ru | следующий шаг |
| Сайт-заготовка `ai` | скрипт готов |
| Домен сайта 2 | позже (reg.ru) |

## Команды на VPS (root)

### HTTPS для 5mb2 (сейчас)

```bash
curl -fsSL https://raw.githubusercontent.com/attack444/AI-assistent/main/project/deploy/install-ssl-5mb2.sh | bash
```

Проверка: https://5mb2.ru/

Если HTTPS странно себя ведёт — у reg.ru убери лишний **AAAA** (IPv6 на старый хостинг), оставь A → VPS.

### Обновить панель (файлы/чат по сайту)
