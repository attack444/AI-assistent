...
echo "==> меню + страницы (mb2_ensure_site_structure)"
# сбросить метку версии — init/php пересоздаст меню
php -r "
...
if (function_exists('mb2_ensure_site_structure')) {
  ...
  update_option('mb2_structure_ver', defined('MB2_THEME_VER') ? MB2_THEME_VER : '1.3.0', false);
  echo 'structure ok, theme=' . get_stylesheet() . PHP_EOL;
} else {
  echo \"functions not loaded — открой сайт один раз\n\";
}
" 2>/dev/null || echo "(php/wp-load пропуск — открой http://5mb2.ru/ один раз)"
